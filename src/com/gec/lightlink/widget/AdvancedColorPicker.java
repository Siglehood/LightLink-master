package com.gec.lightlink.widget;

import java.util.Arrays;
import java.util.List;

import com.gec.lightlink.R;
...
/**
 * A Color picker class that display a wheel of default colors at the top, and a
 * set of draggable seekbars below/to the right (depending on orientation).
 */
public class AdvancedColorPicker extends LinearLayout {
	...
	private final static List<Integer> DEFAULT_COLORS = Arrays.asList(Color.rgb(215, 57, 32), Color.rgb(255, 134, 5),
			Color.rgb(255, 203, 19), Color.rgb(95, 173, 71), Color.rgb(84, 201, 168), Color.rgb(33, 161, 222),
			Color.rgb(16, 36, 87), Color.rgb(91, 32, 103), Color.rgb(212, 221, 228), Color.rgb(0, 0, 0));
	...
	private OnWedgeSelectedListener mWedgeListener = null;
	private OnHsvSeekBarChangedListener mHsvListener = null;
		...
		wheel.addListener(new Wheel.WheelListener() {
			...
			public void onChange(int index) {
				...
				if (mCurrentMode == HSV) {
					...
					mWedgeListener.onWedgeSelected(index);
				} else {
					mAnimator = new ColorAnimator(
							new int[] { Color.red(mColor), Color.green(mColor), Color.blue(mColor) },
	...
	public void setOnWedgeSelectedListener(OnWedgeSelectedListener listener) {
	...
	private Bar findBar(final int seekbarId, final int labelId, final int type) {
		...
		border.setShadowShader(new LinearGradient(0f, 0f, 0f, 50,
				new int[] { Color.argb(100, 0, 0, 0), Color.argb(0, 0, 0, 0) }, null, Shader.TileMode.CLAMP));

		final Bar bar = new Bar((SeekBar) findViewById(seekbarId), (EditText) findViewById(labelId));
		if (type != HUE) {
		...
		bar.label.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {
				...
				switch (type) {
				case HUE:
					setHSV(val, mHSV[1], mHSV[2]);
					break;
				case SAT:
					setHSV(mHSV[0], val / 100f, mHSV[2]);
					break;
				case VAL:
					setHSV(mHSV[0], mHSV[1], val / 100f);
					break;
				default:
					switch (type) {
					case RED:
						setRGB((int) val, Color.green(mColor), Color.blue(mColor));
						break;
					case BLUE:
						setRGB(Color.red(mColor), Color.green(mColor), (int) val);
						break;
					case GREEN:
						setRGB(Color.red(mColor), (int) val, Color.blue(mColor));
						break;
					}
				}
			}
		...
		bar.seekbar.setOnSeekBarChangeListener(new android.widget.SeekBar.OnSeekBarChangeListener() {
			...
			public void onProgressChanged(android.widget.SeekBar seekBar, int progress, boolean fromUser) {
				...
				switch (type) {
				case HUE:
					setHSV(progress / 100f * 360f, mHSV[1], mHSV[2]);
					mHsvListener.onHueChanged((byte) (progress / 100f * 256f));
					break;
				case SAT:
					setHSV(mHSV[0], 1.0f - progress / 100f, mHSV[2]);
					mHsvListener.onSatChanged((byte) ((1.0f - progress / 100f) * 256f));
					break;
				case VAL:
					setHSV(mHSV[0], mHSV[1], progress / 100f);
					mHsvListener.onValChanged((byte) (progress / 100f * 256f));
					break;
				default:
					switch (type) {
					case RED:
						setRGB((int) (progress / 100f * 255f), Color.green(mColor), Color.blue(mColor));
						break;
					case BLUE:
						setRGB(Color.red(mColor), Color.green(mColor), (int) (progress / 100f * 255f));
						break;
					case GREEN:
						setRGB(Color.red(mColor), (int) (progress / 100f * 255f), Color.blue(mColor));
						break;
					}
				}
			}
	...
	public void setOnHsvSeekBarChangedListener(OnHsvSeekBarChangedListener listener) {
	...
	private void updateGradients(float[] hsv) {
		...
		mSatBar.seekbar
				.setGradientColors(
						new int[] { Color.HSVToColor(new float[] { hsv[0], 1, hsv[2] }),
								Color.HSVToColor(new float[] { hsv[0], 0, hsv[2] }), },
						mSatBar.seekbar.getMeasuredWidth());
		mValBar.seekbar
				.setGradientColors(
						new int[] { Color.HSVToColor(new float[] { hsv[0], hsv[1], 0 }),
	...
	private class ColorAnimator extends Animator<int[]> {
		...
		protected void stepAnimation(float dt) {
			setColor(Color.rgb((int) (mStart[0] + (mEnd[0] - mStart[0]) * dt),
					(int) (mStart[1] + (mEnd[1] - mStart[1]) * dt), (int) (mStart[2] + (mEnd[2] - mStart[2]) * dt)));
		}
	}
	...
	private class HSVAnimator extends Animator<float[]> {
		...
		@Override
		protected void stepAnimation(float dt) {
			setHSV(mStart[0] + (mEnd[0] - mStart[0]) * dt, mStart[1] + (mEnd[1] - mStart[1]) * dt,
