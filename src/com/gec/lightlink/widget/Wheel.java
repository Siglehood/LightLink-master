...
/**
 * View group that lays out items in a cirlce. Dragging on the view will rotate
...
public class Wheel extends ViewGroup {
	...
	// Make these DPI independent
	private float MAX_OFFSET = 40; // Number of pixels that selected view is
									// "offset" by the path builder
	...
	private WheelListener mListener; // Listener to be notified when the wheel
										// changes

	// These is pretty specific to the color picker.. Need a better
	// solution
	private Paint mCenterPaint;// Paint a dot in the center of the view
	private int mInnerWidth; // Width of an inner circle. Used to draw inner
								// circle

	private double mStartDragRotation = 0; // The initial rotation when a user
											// starts dragging
	private int mRadius; // The radius of slices. Determined by where the center
							// is positioned
	...
	private Animator<?> mAnimator; // An animator to use when snapping back to a
									// selected position
	private PathBuilder mPathBuilder; // Builds a clip path for the view. By
										// default views are clipped to a wedge
	...
	public static class WedgeBuilder extends PathBuilder {
		...
		private Path buildPath(float angle, int w, int h, float offset) {
			...
			mBasePath.moveTo((int) (iw * cos), (int) (iw * sin));
			mBasePath.lineTo((int) (w * cos), (int) (w * sin));
			mBasePath.arcTo(new RectF(-w, -w, w, w), angle, -2f * angle);
			mBasePath.lineTo((int) (iw * cos), (int) (-1 * iw * sin));
	...
	private double getAngle(MotionEvent event) {
		double val = Math.atan((event.getX() - getMeasuredWidth() * mCx) / (event.getY() - getMeasuredHeight() * mCy))
				* 180 / Math.PI;
	...
	public boolean drawChild(Canvas canvas, View child, long drawingTime) {
		...
		// This interface is too specific to our wedges
		if (mPathBuilder != null) {
